...
func TestLoadInboxEntriesKeepsApprovedFixJobForOpenItem(t *testing.T) {
	tempRoot := t.TempDir()
	originalNewPaths := newPaths
	t.Cleanup(func() {
		newPaths = originalNewPaths
	...
	if err := database.UpdateFixJob(job.ID, db.FixJobUpdate{
		Status:       db.FixJobStatusRunning,
		Phase:        db.FixJobPhaseWaitingForPR,
		Message:      "waiting for no-mistakes pipeline to finish",
		WorktreePath: filepath.Join(tempRoot, "fix-worktree"),
	}); err != nil {
	...
	entry := entries[0]
	if entry.RecommendationID != rec.ID || entry.OptionID != option.ID {
		t.Fatalf("entry recommendation/option = %q/%q, want %q/%q", entry.RecommendationID, entry.OptionID, rec.ID, option.ID)
	}
	if entry.FixStatus != string(db.FixJobStatusRunning) || entry.FixPhase != string(db.FixJobPhaseWaitingForPR) {
		t.Fatalf("entry fix status = %q/%q, want running/waiting_for_pr", entry.FixStatus, entry.FixPhase)
	}
	if entry.DraftComment != option.DraftComment || entry.FixPrompt != option.FixPrompt {
		t.Fatalf("entry option fields = comment %q prompt %q", entry.DraftComment, entry.FixPrompt)
	}
}
...
func TestLoadInboxEntriesTreatsConfigSourceReposAsConfigured(t *testing.T) {
	tempRoot := t.TempDir()
	originalNewPaths := newPaths
	t.Cleanup(func() {
		newPaths = originalNewPaths
	...
	if entries[0].Unconfigured {
		t.Fatalf("config-source repo entry unexpectedly marked unconfigured: %#v", entries[0])
	}
}

func TestLoadInboxEntriesUsesCurrentSyncSnapshotOverPersistedDynamicRepos(t *testing.T) {
	tempRoot := t.TempDir()
	originalNewPaths := newPaths
	originalReadDaemonStatus := readDaemonStatus
	...
	if !configured["dynamic/current"] {
		t.Fatalf("current dynamic repo entry marked unconfigured: %#v", entries)
	}
	if configured["dynamic/stale"] {
		t.Fatalf("stale dynamic repo entry marked configured: %#v", entries)
	}
}
...
func TestApproveInboxEntriesQueuesFixRequiredRecommendationWithComment(t *testing.T) {
	tempRoot := t.TempDir()
	originalNewPaths := newPaths
	originalNewApprovalExecutor := newApprovalExecutor
	t.Cleanup(func() {
	...
	configPath := filepath.Join(tempRoot, "config.yaml")
	if err := config.SaveGlobal(configPath, &config.GlobalConfig{Agent: config.AgentCodex, Fixes: config.FixesConfig{PRCreate: config.PRCreateGH}}); err != nil {
		t.Fatalf("SaveGlobal() error = %v", err)
	}
	...
	if executor.comments[0].Body != "I can reproduce this and will put up a fix." {
		t.Fatalf("comment body = %q", executor.comments[0].Body)
	}
	...
	if len(jobs) != 1 {
		t.Fatalf("fix jobs = %d, want 1", len(jobs))
	}
	if jobs[0].OptionID != rec.Options[0].ID || jobs[0].Status != db.FixJobStatusQueued {
		t.Fatalf("fix job = %#v, want queued job for approved option", jobs[0])
	}
	if jobs[0].FixPrompt != "Fix https://github.com/acme/widgets/issues/42 by adding a regression test." {
		t.Fatalf("fix prompt = %q", jobs[0].FixPrompt)
	}
	if jobs[0].Agent != config.AgentCodex || jobs[0].PRCreate != string(config.PRCreateGH) {
		t.Fatalf("fix job config = agent %q pr_create %q, want codex gh", jobs[0].Agent, jobs[0].PRCreate)
	}
