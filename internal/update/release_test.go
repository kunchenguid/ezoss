...
func TestPickReleaseAssets(t *testing.T) {
	assets := []releaseAsset{
		{Name: "ezoss-v1.2.3-darwin-arm64.tar.gz", BrowserDownloadURL: "https://example.com/archive"},
	...
	archive, checksums, err := pickReleaseAssets("ezoss", "v1.2.3", assets, platformSpec{GOOS: "darwin", GOARCH: "arm64"})
	if err != nil {
		t.Fatalf("pickReleaseAssets error = %v", err)
	}
	if archive.Name != "ezoss-v1.2.3-darwin-arm64.tar.gz" {
		t.Fatalf("archive = %q", archive.Name)
	}
	if checksums.Name != checksumsAssetName {
		t.Fatalf("checksums = %q", checksums.Name)
	}

	_, _, err = pickReleaseAssets("ezoss", "v1.2.3", assets[:1], platformSpec{GOOS: "darwin", GOARCH: "arm64"})
	if err == nil {
...
func TestParseChecksums(t *testing.T) {
	archiveName := "ezoss-v1.2.3-darwin-arm64.tar.gz"
	data := []byte("abc123  ezoss-v1.2.3-darwin-arm64.tar.gz\ndef456  other.tar.gz\n")
...
func TestVerifyChecksum(t *testing.T) {
	...
	if err := verifyChecksum(payload, strings.Repeat("0", 64)); err == nil {
